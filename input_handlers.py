...
class EventHandler(tcod.event.EventDispatch[Action]):
    ...
    def handle_action(self, action: Optional[Action]) -> bool:
        ...
        Returns true if action will advance a turn, false if not."""
        if action is None:
            return False
        try:
            action.perform()
    ...
    def ev_mouse_motion(self, event: tcod.event.MouseMotion) -> None:
...
class AskUserEventHandler(EventHandler):
    ...
    def handle_action(self, action: Optional[Action]) -> bool:
        """Return to the main event handler when the user is done."""
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.console.Console) -> None:
        ...
        Will move to a different position based on where the player is located, so the always see  where they are."""
